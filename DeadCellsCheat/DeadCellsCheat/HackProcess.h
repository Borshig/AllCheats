...
public:
	...
	void RunProcess(const char* mainExec, const char* windowName)
	{
		...
		runSetDebugPrivs();
		while (!FindProcessName(mainExec, &__gameProcess)) SleepTime(12); //"hl2.exe"
		...
		while (__dwordClient == 0x0) __dwordClient = GetModuleNamePointer(mainExec, __gameProcess.th32ProcessID);
		//while (__dwordThreadStack0 == 0x0) __dwordThreadStack0 = GetModuleNamePointer("THREADSTACK0", __gameProcess.th32ProcessID);
	...
	//	while(__dwordSteam == 0x0) __dwordSteam = GetModuleNamePointer("steam.dll", __gameProcess.th32ProcessID);
		__HWND = FindWindowA(nullptr, windowName);

		return;
	}
};

class CHackProcess64 : public CHackProcess32
{
public:
...
extern CHackProcess32 fProcess32;
extern CHackProcess64 fProcess64;
