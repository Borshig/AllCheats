...
public:
	...
	void RunProcess()
	{
		...
		ListProcessModules(__gameProcess.th32ProcessID);
		while (__dwordEngine == 0x0) __dwordEngine = GetModuleNamePointer("soulstorm.exe", __gameProcess.th32ProcessID);
		//while (__dwordWXPModDLL == 0x0) __dwordWXPModDLL = GetModuleNamePointer("WXPMod.dll", __gameProcess.th32ProcessID);
