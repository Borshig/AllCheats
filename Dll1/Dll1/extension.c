            ...
            if (!mysql_stmt_bind_param(stmt, params))
            {
                if (!mysql_stmt_execute(stmt))
                {
                    ...
                    if (columnNum > 0)
                    {
                        ...
                        MYSQL_BIND result[columnNum];

                        memset(result, 0, sizeof (MYSQL_BIND) * columnNum);

                        char userData[columnNum][256];
                        char nulls[columnNum];
    ...
    else if (strcmp(function, "connect") == 0)
    {
        if (argc == 5)
        {
            for (int i = 0; i < argc; ++i)
            {
                ...
                {
                    char msg[30];
                    sprintf(msg, "argument %d is too long", i);
            ...
            {
                char msg[1024];
                sprintf(msg, "failed to connect to MySQL: %s", mysql_error(connection));
                reply(output, eSystem, msg);
            }
        }
        else
